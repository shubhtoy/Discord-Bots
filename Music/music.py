...
class Downloader(discord.PCMVolumeTransformer):
    ...
    async def yt_download(cls, url, ytdl, *, loop=None, stream=False):
        ...
        download = await loop.run_in_executor(
            None, lambda: ytdl.extract_info(song_url, download=not stream)
        )
        filename = data["url"] if stream else ytdl.prepare_filename(download)
        return (
            cls(discord.FFmpegPCMAudio(filename, **ffmpeg_options), data=download),
            song_info,
        )
...
class MusicPlayer(commands.Cog, name="Music"):
    ...
    async def music_voice(self, user, before, after):
        ...
        if after.channel is None and user.id == self.bot.user.id:
            try:
                ...
                print(
                    f"Failed to get guild id {user.guild.id}"
    ...
    async def voice_check(self, msg):
        ...
        if msg.voice_client is not None:
            ...
            if (
                msg.voice_client is not None
                and msg.voice_client.is_playing() is False
                and msg.voice_client.is_paused() is False
            ):
    ...
    async def done(self, msg, msgId: int = None):
        ...
        if self.player[msg.guild.id]["queue"]:
            ...
            return await self.start_song(
                msg=queue_data["author"], song=queue_data["title"]
            )
    ...
    async def start_song(self, msg, song):
        ...
        #     msg.voice_client
        # ource.volume=self.music[str(msg.guild.id)]['vol']/100
        msg.voice_client.source.volume = self.player[msg.guild.id]["volume"]
    ...
    async def play(self, msg, *, song):
        ...
        if msg.guild.id in self.player:
            ...
            if (
                msg.voice_client.is_playing() is False
                and not self.player[msg.guild.id]["queue"]
            ):
    ...
    async def reset(self, msg):
        ...
        if msg.voice_client is None:
            return await msg.send(
                f"**{msg.author.display_name}, there is no audio currently playing from the bot.**"
            )

        if (
            msg.author.voice is None
            or msg.author.voice.channel != msg.voice_client.channel
        ):
            return await msg.send(
                f"**{msg.author.display_name}, you must be in the same voice channel as the bot.**"
            )
        ...
        if (
            ...
            return await msg.send(
                "**No audio currently playing or songs in queue**".title(),
                delete_after=25,
            )
    ...
    async def skip(self, msg):
        ...
        if msg.voice_client is None:
            return await msg.send(
                "**No music currently playing**".title(), delete_after=60
            )
        ...
        if (
            ...
            return await msg.send(
                "**No songs in queue to skip**".title(), delete_after=60
            )
    ...
    async def stop(self, msg):
        ...
        if msg.author.voice is not None and msg.voice_client is not None:
            if (
                msg.voice_client.is_playing() is True
    ...
    async def leave(self, msg):
        ...
        if msg.author.voice is not None and msg.voice_client is not None:
            ...
            return await msg.voice_client.disconnect(), await msg.message.add_reaction(
                emoji="✅"
            )

        if msg.author.voice is None:
            return await msg.send(
                "You must be in the same voice channel as bot to disconnect it via command"
            )
    ...
    async def song_info(self, msg):
        ...
        return await msg.send(
            f"**No songs currently playing**".title(), delete_after=30
        )
    ...
    async def join_error(self, msg, error):
        ...
        if error.args[0] == "Command raised an exception: Exception: playing":
            return await msg.send(
                "**Please join the same voice channel as the bot to add song to queue**".title()
            )
    ...
    async def volume(self, msg, vol: int):
        ...
        if msg.author.voice is not None:
            if msg.voice_client is not None:
                if (
                    msg.voice_client.channel == msg.author.voice.channel
                    and msg.voice_client.is_playing() is True
                ):
    ...
    async def volume_error(self, msg, error):
        if isinstance(error, commands.MissingPermissions):
            return await msg.send(
                "Manage channels or admin perms required to change volume",
                delete_after=30,
            )
